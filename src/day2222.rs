use std::cmp::max;
use std::collections::HashMap;

pub fn run(part: i8) {
    if part == 1 {
        part1();
    } else {
        part2();
    }
}

fn part1() {
    let mut map = Map::parse();
    map.walk();
    println!("{}", map.password());
}

fn part2() {
}

#[derive(Debug, Copy, Clone, Hash, Eq, PartialEq)]
struct Coord {
    row: usize,
    col: usize,
}

#[derive(Debug)]
enum Square {
    Open,
    Wall,
}

#[derive(Debug)]
enum Dir {
    Right, // 0
    Down,  // 1
    Left,  // 2
    Up,    // 3
}

#[derive(Debug)]
enum Instruction {
    Move(usize),
    TurnLeft,
    TurnRight,
}

#[derive(Debug)]
struct Map {
    squares: HashMap<Coord, Square>,
    pos: Coord,
    dir: Dir,
    instructions: Vec<Instruction>,
    max_row: usize,
    max_col: usize,
}

impl Map {
    fn walk(&mut self) {
        for instruction in &self.instructions {
            match instruction {
                Instruction::TurnLeft => {
                    match self.dir {
                        Dir::Right => self.dir = Dir::Up,
                        Dir::Up => self.dir = Dir::Left,
                        Dir::Left => self.dir = Dir::Down,
                        Dir::Down => self.dir = Dir::Right,
                    }
                },
                Instruction::TurnRight => {
                    match self.dir {
                        Dir::Right => self.dir = Dir::Down,
                        Dir::Up => self.dir = Dir::Right,
                        Dir::Left => self.dir = Dir::Up,
                        Dir::Down => self.dir = Dir::Left,
                    }
                },
                Instruction::Move(steps) => {
                    for _ in 0..*steps {
                        let next_coord = match self.dir {
                            Dir::Up => {
                                let mut c = Coord { row: if self.pos.row > 1 { self.pos.row - 1 } else { self.max_row }, col: self.pos.col, };
                                while !self.squares.contains_key(&c) {
                                    c = Coord { row: if c.row > 1 { c.row - 1 } else { self.max_row }, col: c.col, };
                                }
                                c
                            },
                            Dir::Down => {
                                let mut c = Coord { row: if self.pos.row == self.max_row { 1 } else { self.pos.row+1 }, col: self.pos.col, };
                                while !self.squares.contains_key(&c) {
                                    c = Coord { row: if c.row == self.max_row { 1 } else { c.row+1 }, col: c.col, };
                                }
                                c
                            },
                            Dir::Left => {
                                let mut c = Coord { col: if self.pos.col > 1 { self.pos.col - 1 } else { self.max_col }, row: self.pos.row, };
                                while !self.squares.contains_key(&c) {
                                    c = Coord { col: if c.col > 1 { c.col - 1 } else { self.max_col }, row: c.row, };
                                }
                                c
                            },
                            Dir::Right => {
                                let mut c = Coord { col: if self.pos.col == self.max_col { 1 } else { self.pos.col+1 }, row: self.pos.row, };
                                while !self.squares.contains_key(&c) {
                                    c = Coord { col: if c.col == self.max_col { 1 } else { c.col+1 }, row: c.row, };
                                }
                                c
                            },
                        };

                        match self.squares.get(&next_coord).unwrap() {
                            Square::Open => self.pos = next_coord,
                            Square::Wall => break,
                        }
                    }
                }
            }
        }
    }

    fn password(&self) -> usize {
        self.pos.row * 1000 + self.pos.col * 4 + match self.dir {
            Dir::Right => 0,
            Dir::Down => 1,
            Dir::Left => 2,
            Dir::Up => 3,
        }
    }

    fn parse() -> Self {
        let mut squares = HashMap::new();
        let dir = Dir::Right;
        let mut pos = Coord{ row: 0, col: 0 };
        let mut next_instructions = false;
        let mut instructions = "";
        let mut max_row = 0;
        let mut max_col = 0;

        for (j, line) in input().lines().enumerate() {
            let line = line.trim_end();
            if next_instructions {
                instructions = line;
                break;
            } else if line.is_empty() {
                next_instructions = true;
                continue;
            }

            let row = j + 1;
            for (i, c) in line.chars().enumerate() {
                if c != '.' && c != '#' {
                    continue;
                }
                let col = i + 1;
                let square = match c {
                    '#' => Square::Wall,
                    _ => Square::Open,
                };
                let coord = Coord { row, col };
                if pos.row == 0 {
                    pos = coord;
                }
                max_row = max(max_row, row);
                max_col = max(max_col, col);
                squares.insert(coord, square);
            }
        }

        // parse instructions
        let mut i = vec![];
        let mut tok_start: usize = 0;
        let instructions: Vec<char> = instructions.chars().collect();
        while tok_start < instructions.len() {
            match instructions[tok_start] {
                'R' => {
                    i.push(Instruction::TurnRight);
                    tok_start += 1;
                },
                'L' => {
                    i.push(Instruction::TurnLeft);
                    tok_start += 1;
                },
                _ => {
                    let mut tok_end: usize = tok_start + 1;
                    while tok_end < instructions.len() && instructions[tok_end] >= '0' && instructions[tok_end] <= '9' {
                        tok_end += 1;
                    }
                    let mut num = "".to_string();
                    for i in tok_start..tok_end {
                        num += &instructions[i].to_string();
                    }
                    i.push(Instruction::Move((&num).parse::<usize>().unwrap()));
                    tok_start = tok_end;
                }
            }
        }

        Self { squares, dir, pos, instructions: i, max_row, max_col, }
    }
}

fn input_test() -> &'static str {
    r###"
        ...#
        .#..
        #...
        ....
...#.......#
........#...
..#....#....
..........#.
        ...#....
        .....#..
        .#......
        ......#.

10R5L5R10L4R5L5
    "###.trim()
}

fn input() -> &'static str {
    r###"
                                                  ..................#..#.....#....................#............#.......#..##..........................
                                                  .#...#.....#.........................................#...##...#...#....#...................#........
                                                  ...........##.......#.........#.#.#...#..#.............#.............#........#.................##..
                                                  ..#........#.......#.............................................#......#...#.......................
                                                  ..............................#..........#...................#..................#.............#.....
                                                  .......#..#.....#.......#...#.........................#.........#...#..#...............#........##..
                                                  .........#.....#...............#...##.........#...................#..#..##.....#...........#.....#..
                                                  ..#.#.......#............#.#.............#...................#......................##..............
                                                  #......#......#......................#.......#.........#..............#.....#.#....#...#..#.........
                                                  #.#.......#.............................#.........................#.........#.#..#............#.....
                                                  #.................#...........#...#...#.....................................#............##...#.#...
                                                  ............#.....##............................##............#..#...#.......#..#..............#....
                                                  ....##..............#.................#...............#....#..#...#..............#.........#........
                                                  ......#..#...................................#.#......#..#....#............#..#......#..............
                                                  .........#........................................#........#....##....................#...........#.
                                                  .............#....##.#..#...#...##.#..................#.........#.##................................
                                                  ...............#...........#.........##................#............................................
                                                  #.###.#..#.........#......#........#......#......#.......................#.....................#....
                                                  ......................#................#.........#....#..##.....##....#....#........................
                                                  ...#.#.............................#.....##...#.#......#.......#.#..........#.###................#..
                                                  ......#....#.............#...............#.........#................#..#...........#......##......#.
                                                  .#.....................#...........#..............#...........................................##....
                                                  ....#...................................#...........................#..............#...........#....
                                                  ......................................#..........................#.............##...............#...
                                                  ###.......#.....#..#...........#..#.#.....#......#..........#........#.....#..#......#..............
                                                  .................#....#.............................#..................#.##.....#...................
                                                  ...............#...........................#....#......#..#...........#.............................
                                                  .........#....#........#.....................#.......#.#.#................#.........................
                                                  .........##..#....................................#.........#...#...#.....#.#......#................
                                                  ...##.......................#...........#...#........#...#.###....................#..........#......
                                                  #....................#......#...#....................#................#.#........##.........#......#
                                                  .....................................#................#..........#......#.........#...#.............
                                                  .....................#.....#..........................................#............#................
                                                  ....#..#..#........#..........#....................#....#...#..#..............##................#...
                                                  ............#.#...........##.............................#.............#..#.................#.......
                                                  .....#........#...##...............................#............#...#....#...#............##........
                                                  .....#...#.......#....#....#....#...................#...........#................#.........#........
                                                  .#.#.......#.#..........#..................#.........#....#...............................#.........
                                                  .........#..#.............#......#......#......#........#........#.#.#.................#....##......
                                                  .........#....#......................#.#.............###...#...#........#.......#....#..##..........
                                                  ...................##......#.........#...#...................#..................#...................
                                                  ..........#..........#....#.........#...................#............#..#...........................
                                                  ..........................#...............#....#...........#.................#.........#.#...#.#....
                                                  ......#....#...................#........................#........#......#...#......#................
                                                  #..................................#....#................#.#..............#.........................
                                                  .....#...#......................................#.....................#.....#......#........#.....#.
                                                  ..#.....#.......#..#...#...............................#.......#..................#......#.#........
                                                  ....................#.......#........#.........................##..#.........#.#.#........#....#....
                                                  ........#............#.......................................#........##............................
                                                  ....#..............#...........#....##............#.............#.....................#.............
                                                  .#........#.........#.....#.#...#..............#.#
                                                  ..#......#....................#.##........#...#...
                                                  ............#.......##......##........#...........
                                                  ......#.#.........#.......................#.......
                                                  ............#...................................#.
                                                  ............................#.#..............#....
                                                  .........................................#........
                                                  ............#.......#....#..#................#....
                                                  ...........#.#............#.....................#.
                                                  ..........#...#.............................#.....
                                                  ....................................#.#..##.......
                                                  ..#....#............#.....#.......................
                                                  .....#...#...........#............................
                                                  ...............#.......#..........#.....#.........
                                                  ..................................................
                                                  .....#.##.......#.##.....#.#.#...#................
                                                  ...........#..##.....#..................#.........
                                                  ....................#.......................#.....
                                                  .......#...##...............................#...##
                                                  ......#..............#........#.........#....#....
                                                  #.#.##...........#.......#..#........#...........#
                                                  ............##.................#.##...........#...
                                                  ...#.......#................#.#..#.#..............
                                                  ..#...........#............#..#.#......#.........#
                                                  ..................................................
                                                  ...........#..............#..............#........
                                                  ....#.#..............##.......#.#......#...#.#....
                                                  .........##.............#...#...#.................
                                                  ...#......#.....#...#..#..##..........#...........
                                                  .........#............#...........................
                                                  ......................#....#.........#............
                                                  .......................#...............#.......#..
                                                  ..........#.#....#............##................##
                                                  ..#......#..........#..#......#.......#.........#.
                                                  ................#.#...#.......#........#..........
                                                  ....#.....#..............................#........
                                                  ..............#............#...................#..
                                                  ....#.......#..#.....#...........#.....#......#...
                                                  ......#.....................#......#.#............
                                                  .....#.#....................#..........##.....#...
                                                  #...........#........#............................
                                                  ..#..........#..#...........#....................#
                                                  ...#.#.........#.........#.....#............#.....
                                                  ...#.........................#....#.#......#......
                                                  ....#.....#..............#.............#..........
                                                  ...#........#...........##.......................#
                                                  #....#.......#......#...#..........#..........#...
                                                  ........#...#.......#.....#...#................#..
                                                  #..............#.#....#..#...................#....
                                                  .........#.....#.#................................
#....##................#..........#...#.#......#...........................#.........#...........#..
#..........................#..........................#......#.......##.#...........................
#.......##...#.......#........#.....#......#...#.#....#.....#.......#.............#.................
.#......##.....#.#.........#........#...#.......................................#...........#.....#.
........#..###............#......##..#............#.......#.....#....#.......#......................
....#.#.....#...#....#.....#...#..#.#...........#..............#..................#............#....
..................#........#..#.....#..#...#....##..............#..##...#.......#.....#.............
......#...#.##......#......#.....#....#..........#.....#.....#...............................##.....
.......................#.....##.......#....#....................................#.................#.
.........#........................#..........#...#......#.....##....................................
.......#........#...................#.......#...#.............#.#..........#.#...#..#.....#.....#...
.....#..............#................#.....#........................#....#....#.....................
##...#.#..#..........#.................#....##......#.......#..................#.......#..#..#......
#............#...#.................#.................#.#..#.......#.#...............#...............
.#...........#...#.........................#........#.......##..................#....#..............
#......#.........#...................#........#........#.........#.............#.........#..........
.................................#...#......................#........#...#....#.....................
#..........#...........................#.............#....##.............###.........#........#.....
..............#..##...#..............#.......#............#...#.....#............#....#......#.#....
.........#.....#......#.....................................#.............#.......##..#.......#.....
..#..#.#......................#....#..........#...........#............................#....#...#...
#............#..............#...#...##.#.##........#..#....#....#...#...............#.#.............
..#...............#....#...#.........#..#......#..............#........#.........#..................
...........#....................#.....#...#...#..........#..........................................
......................#.............#....#..............#.......#....#.#...........................#
.................#.......#...#.......#...................#.#.......#.....#..........................
#............................#..#..........#..............##..#.............#..........#............
..................#.....................................#..#.#................................#....#
.#......#......#.......#........#.............#..#....................................#.....#..#....
........#..#.......................................#............#.....##.........#...........#..#...
......#..............#..#..#..#.............#..................#...#...........................#....
......................#...#..#.......#.#....................#...........#.......................#...
..#...........#..................#..#....#............................#....#...#.............##..#..
......#....#....#.#.......#...#...............#..........#..........#....#...#....................#.
......................................#................................#...#...........#....#..#....
................#.......................#.......................##.#........#...#............#......
......#..#.................#..........#......................#............#....#..#.......#.........
..##..............................#..#....#.................#........#.#.......#....................
...##.#....#..#..............#......#.......#..#.#.#.......#.#.......##..........#.....#..........##
......#..#....#....#..#.......#............#........###..........#.#..#...........#..........#..#..#
...#........#..........#....................#.........#.....................#...................#...
.#.#........#.................................#..............................#......#...............
.........#...........##........#..............##........#..........#.................#..............
......###....##...#.......................#....................#...................##.#..........#..
..............#....#........................................#.#.............#.....#.#....##.......#.
.........................................................................#.#.##.#..#.....#..........
...#.........#.........#......#..................#....................#..........................#..
..#...#.....#.............#..##..............................#.........##.#..#......................
.....................#...........#.....#......................#..............##..........#.......##.
.............#...............##.................#...#.........##........##.........................#
...#..............................................
.#.#..#...........#.............#.#......##......#
.................................#........##..#.#.
.......................#......#.........#..#......
..#.............#.#................#..............
.........##.##....................................
......#..##.............#....#.........#..........
...............#...........#..............##......
.........#.......#.#..#................#.....#..#.
.#..#.......................#.....#.........#..#..
.................#..#...................##.#..#...
........#...............#...##.............#..##..
...............#..........#.......................
..........#......#..#...................#.......#.
.#.......#..#..........................#..#.......
#.......#............................#............
.....#.#........#.........###............#........
.#.............#............##..#...#...#.#.......
.........#.......#..........#...#........#.#......
...#...........#...............#..................
........#................................#........
............#................................#....
....#........................#..##......#...#.....
.#.##..............#.#..................#.#...#.#.
.............#..........#...................#.....
..##....#....#..#...........#....#.##.............
.#.........#....................#...............#.
........#.#..........#...............#...#...#...#
...............#.#..................#.....##......
.....#....#.##..........#.......#....#...#....#.#.
..#.............#......#............#......#..#...
..#..#............#.#................#..#.........
.........#.......#......#......................#..
..........#.........................#.............
.##.......#...................#.........#..#......
...........#.....##.....#.#...#..........###......
...........#.......#..............................
.........#.......#...#.....#..........#....#..#...
......###.#....#.#..........##....#.......##.#....
.......#.............#..................#.....#...
.............#..........#......#......#...........
...#...........###.....#..#.................#.#...
..................#........##....#......#..#......
........................#.........##.....##.......
........#..#.#....#..........#...#...........#....
.....#........##........#............#..#.........
....................#............#.......#........
.................#.#...#....#.........#..##.......
...........#..........##....#..........#.....#....
..............#..........#.............#..........


    "###.trim()
}
