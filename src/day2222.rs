use std::collections::HashSet;

pub fn run(part: i8) {
    if part == 1 {
        part1();
    } else {
        part2();
    }
}

fn part1() {
    let mut map = Map::parse();
    println!("ROWS");
    println!("{:#?}", map.rows);
    println!("COLS");
    println!("{:#?}", map.cols);
    map.walk();
    println!("PASSWORD");
    println!("{}", map.password());
}

fn part2() {
}

#[derive(Debug, Copy, Clone, Hash, Eq, PartialEq)]
struct Coord {
    row: usize,
    col: usize,
}

#[derive(Debug)]
enum Dir {
    Right, // 0
    Down,  // 1
    Left,  // 2
    Up,    // 3
}

#[derive(Debug)]
enum Instruction {
    Move(usize),
    TurnLeft,
    TurnRight,
}

#[derive(Debug)]
struct Row {
    first_col: usize,
    last_col: usize,
    wall_cols: HashSet<usize>,
}

#[derive(Debug)]
struct Col {
    first_row: usize,
    last_row: usize,
    wall_rows: HashSet<usize>,
}

#[derive(Debug)]
struct Map {
    rows: Vec<Option<Row>>,
    cols: Vec<Option<Col>>,

    pos: Coord,
    dir: Dir,
    instructions: Vec<Instruction>,
}

impl Map {
    fn walk(&mut self) {
        for instruction in &self.instructions {
            match instruction {
                Instruction::TurnLeft => {
                    match self.dir {
                        Dir::Right => self.dir = Dir::Up,
                        Dir::Up => self.dir = Dir::Left,
                        Dir::Left => self.dir = Dir::Down,
                        Dir::Down => self.dir = Dir::Right,
                    }
                },
                Instruction::TurnRight => {
                    match self.dir {
                        Dir::Right => self.dir = Dir::Down,
                        Dir::Up => self.dir = Dir::Right,
                        Dir::Left => self.dir = Dir::Up,
                        Dir::Down => self.dir = Dir::Left,
                    }
                },
                Instruction::Move(steps) => {
                    for _ in 0..*steps {
                        let next_coord = match self.dir {
                            Dir::Up => {
                                let col = self.cols.get(self.pos.col).unwrap().as_ref().unwrap();
                                Coord {
                                    row: if col.first_row == self.pos.row { col.last_row } else { self.pos.row-1 },
                                    col: self.pos.col,
                                }
                            },
                            Dir::Down => {
                                let col = self.cols.get(self.pos.col).unwrap().as_ref().unwrap();
                                Coord {
                                    row: if col.last_row == self.pos.row { col.first_row } else { self.pos.row+1 },
                                    col: self.pos.col,
                                }
                            },
                            Dir::Left => {
                                let row = self.rows.get(self.pos.row).unwrap().as_ref().unwrap();
                                Coord {
                                    row: self.pos.row,
                                    col: if row.first_col == self.pos.col { row.last_col } else { self.pos.col-1 },
                                }
                            },
                            Dir::Right => {
                                let row = self.rows.get(self.pos.row).unwrap().as_ref().unwrap();
                                Coord {
                                    row: self.pos.row,
                                    col: if row.last_col == self.pos.col { row.first_col } else { self.pos.col+1 },
                                }
                            },
                        };

                        // see if next coord will hit a wall
                        let row = self.rows.get(self.pos.row).unwrap().as_ref().unwrap();
                        if row.wall_cols.contains(&next_coord.col) {
                            break;
                        }
                        self.pos = next_coord;
                    }
                }
            }
        }
    }

    fn password(&self) -> usize {
        self.pos.row * 1000 + self.pos.col * 4 + match self.dir {
            Dir::Right => 0,
            Dir::Down => 1,
            Dir::Left => 2,
            Dir::Up => 3,
        }
    }

    fn parse() -> Self {
        let dir = Dir::Right;
        let mut pos = Coord{ row: 0, col: 0 };
        let mut next_instructions = false;
        let mut instructions = "";
        let mut rows: Vec<Option<Row>> = vec![];
        let mut cols: Vec<Option<Col>> = vec![];
        // row/col at index zero isn't used
        rows.push(None);
        cols.push(None);

        // parse grid
        for (j, line) in input().lines().enumerate() {
            if j == 0 {
                println!("FIRST ROW: {line:?}");
            }
            let line = line.trim_end();
            if next_instructions {
                instructions = line;
                break;
            } else if line.is_empty() {
                next_instructions = true;
                continue;
            }

            let row = j + 1;
            let mut rinfo: Row = Row {
                first_col: 0,
                last_col: 0,
                wall_cols: HashSet::new(),
            };

            for (i, c) in line.chars().enumerate() {
                if row == 1 {
                    println!("ROW 1: {i}, {c}");
                }
                if c != '.' && c != '#' {
                    continue;
                }
                let col = i + 1;
                let coord = Coord { row, col };
                if pos.row == 0 && c == '.' {
                    pos = coord;
                }
                if rinfo.first_col == 0 {
                    rinfo.first_col = col;
                }
                rinfo.last_col = col;
                if c == '#' {
                    rinfo.wall_cols.insert(col);
                }

                // add col info
                while cols.len() <= col {
                    cols.push(Some(Col{
                        first_row: 0,
                        last_row: 0,
                        wall_rows: HashSet::new(),
                    }));
                }
                if let Some(cinfo) = cols.get_mut(col).unwrap() {
                    if cinfo.first_row == 0 {
                        cinfo.first_row = row;
                    }
                    cinfo.last_row = row;
                    if c == '#' {
                        cinfo.wall_rows.insert(row);
                    }
                }
            }

            rows.push(Some(rinfo));
        }

        // parse instructions
        let mut i = vec![];
        let mut tok_start: usize = 0;
        let instructions: Vec<char> = instructions.chars().collect();
        while tok_start < instructions.len() {
            match instructions[tok_start] {
                'R' => {
                    i.push(Instruction::TurnRight);
                    tok_start += 1;
                },
                'L' => {
                    i.push(Instruction::TurnLeft);
                    tok_start += 1;
                },
                _ => {
                    let mut tok_end: usize = tok_start + 1;
                    while tok_end < instructions.len() && instructions[tok_end] >= '0' && instructions[tok_end] <= '9' {
                        tok_end += 1;
                    }
                    let mut num = "".to_string();
                    for i in tok_start..tok_end {
                        num += &instructions[i].to_string();
                    }
                    i.push(Instruction::Move((&num).parse::<usize>().unwrap()));
                    tok_start = tok_end;
                }
            }
        }

        Self {
            rows,
            cols,
            pos,
            dir,
            instructions: i,
        }
    }
}

fn input() -> &'static str {
    &r###"
        ...#
        .#..
        #...
        ....
...#.......#
........#...
..#....#....
..........#.
        ...#....
        .....#..
        .#......
        ......#.

10R5L5R10L4R5L5
    "###.trim_end()[1..]
}

fn input_real() -> &'static str {
    &r###"
                                                  ..................#..#.....#....................#............#.......#..##..........................
                                                  .#...#.....#.........................................#...##...#...#....#...................#........
                                                  ...........##.......#.........#.#.#...#..#.............#.............#........#.................##..
                                                  ..#........#.......#.............................................#......#...#.......................
                                                  ..............................#..........#...................#..................#.............#.....
                                                  .......#..#.....#.......#...#.........................#.........#...#..#...............#........##..
                                                  .........#.....#...............#...##.........#...................#..#..##.....#...........#.....#..
                                                  ..#.#.......#............#.#.............#...................#......................##..............
                                                  #......#......#......................#.......#.........#..............#.....#.#....#...#..#.........
                                                  #.#.......#.............................#.........................#.........#.#..#............#.....
                                                  #.................#...........#...#...#.....................................#............##...#.#...
                                                  ............#.....##............................##............#..#...#.......#..#..............#....
                                                  ....##..............#.................#...............#....#..#...#..............#.........#........
                                                  ......#..#...................................#.#......#..#....#............#..#......#..............
                                                  .........#........................................#........#....##....................#...........#.
                                                  .............#....##.#..#...#...##.#..................#.........#.##................................
                                                  ...............#...........#.........##................#............................................
                                                  #.###.#..#.........#......#........#......#......#.......................#.....................#....
                                                  ......................#................#.........#....#..##.....##....#....#........................
                                                  ...#.#.............................#.....##...#.#......#.......#.#..........#.###................#..
                                                  ......#....#.............#...............#.........#................#..#...........#......##......#.
                                                  .#.....................#...........#..............#...........................................##....
                                                  ....#...................................#...........................#..............#...........#....
                                                  ......................................#..........................#.............##...............#...
                                                  ###.......#.....#..#...........#..#.#.....#......#..........#........#.....#..#......#..............
                                                  .................#....#.............................#..................#.##.....#...................
                                                  ...............#...........................#....#......#..#...........#.............................
                                                  .........#....#........#.....................#.......#.#.#................#.........................
                                                  .........##..#....................................#.........#...#...#.....#.#......#................
                                                  ...##.......................#...........#...#........#...#.###....................#..........#......
                                                  #....................#......#...#....................#................#.#........##.........#......#
                                                  .....................................#................#..........#......#.........#...#.............
                                                  .....................#.....#..........................................#............#................
                                                  ....#..#..#........#..........#....................#....#...#..#..............##................#...
                                                  ............#.#...........##.............................#.............#..#.................#.......
                                                  .....#........#...##...............................#............#...#....#...#............##........
                                                  .....#...#.......#....#....#....#...................#...........#................#.........#........
                                                  .#.#.......#.#..........#..................#.........#....#...............................#.........
                                                  .........#..#.............#......#......#......#........#........#.#.#.................#....##......
                                                  .........#....#......................#.#.............###...#...#........#.......#....#..##..........
                                                  ...................##......#.........#...#...................#..................#...................
                                                  ..........#..........#....#.........#...................#............#..#...........................
                                                  ..........................#...............#....#...........#.................#.........#.#...#.#....
                                                  ......#....#...................#........................#........#......#...#......#................
                                                  #..................................#....#................#.#..............#.........................
                                                  .....#...#......................................#.....................#.....#......#........#.....#.
                                                  ..#.....#.......#..#...#...............................#.......#..................#......#.#........
                                                  ....................#.......#........#.........................##..#.........#.#.#........#....#....
                                                  ........#............#.......................................#........##............................
                                                  ....#..............#...........#....##............#.............#.....................#.............
                                                  .#........#.........#.....#.#...#..............#.#
                                                  ..#......#....................#.##........#...#...
                                                  ............#.......##......##........#...........
                                                  ......#.#.........#.......................#.......
                                                  ............#...................................#.
                                                  ............................#.#..............#....
                                                  .........................................#........
                                                  ............#.......#....#..#................#....
                                                  ...........#.#............#.....................#.
                                                  ..........#...#.............................#.....
                                                  ....................................#.#..##.......
                                                  ..#....#............#.....#.......................
                                                  .....#...#...........#............................
                                                  ...............#.......#..........#.....#.........
                                                  ..................................................
                                                  .....#.##.......#.##.....#.#.#...#................
                                                  ...........#..##.....#..................#.........
                                                  ....................#.......................#.....
                                                  .......#...##...............................#...##
                                                  ......#..............#........#.........#....#....
                                                  #.#.##...........#.......#..#........#...........#
                                                  ............##.................#.##...........#...
                                                  ...#.......#................#.#..#.#..............
                                                  ..#...........#............#..#.#......#.........#
                                                  ..................................................
                                                  ...........#..............#..............#........
                                                  ....#.#..............##.......#.#......#...#.#....
                                                  .........##.............#...#...#.................
                                                  ...#......#.....#...#..#..##..........#...........
                                                  .........#............#...........................
                                                  ......................#....#.........#............
                                                  .......................#...............#.......#..
                                                  ..........#.#....#............##................##
                                                  ..#......#..........#..#......#.......#.........#.
                                                  ................#.#...#.......#........#..........
                                                  ....#.....#..............................#........
                                                  ..............#............#...................#..
                                                  ....#.......#..#.....#...........#.....#......#...
                                                  ......#.....................#......#.#............
                                                  .....#.#....................#..........##.....#...
                                                  #...........#........#............................
                                                  ..#..........#..#...........#....................#
                                                  ...#.#.........#.........#.....#............#.....
                                                  ...#.........................#....#.#......#......
                                                  ....#.....#..............#.............#..........
                                                  ...#........#...........##.......................#
                                                  #....#.......#......#...#..........#..........#...
                                                  ........#...#.......#.....#...#................#..
                                                  #..............#.#....#..#...................#....
                                                  .........#.....#.#................................
#....##................#..........#...#.#......#...........................#.........#...........#..
#..........................#..........................#......#.......##.#...........................
#.......##...#.......#........#.....#......#...#.#....#.....#.......#.............#.................
.#......##.....#.#.........#........#...#.......................................#...........#.....#.
........#..###............#......##..#............#.......#.....#....#.......#......................
....#.#.....#...#....#.....#...#..#.#...........#..............#..................#............#....
..................#........#..#.....#..#...#....##..............#..##...#.......#.....#.............
......#...#.##......#......#.....#....#..........#.....#.....#...............................##.....
.......................#.....##.......#....#....................................#.................#.
.........#........................#..........#...#......#.....##....................................
.......#........#...................#.......#...#.............#.#..........#.#...#..#.....#.....#...
.....#..............#................#.....#........................#....#....#.....................
##...#.#..#..........#.................#....##......#.......#..................#.......#..#..#......
#............#...#.................#.................#.#..#.......#.#...............#...............
.#...........#...#.........................#........#.......##..................#....#..............
#......#.........#...................#........#........#.........#.............#.........#..........
.................................#...#......................#........#...#....#.....................
#..........#...........................#.............#....##.............###.........#........#.....
..............#..##...#..............#.......#............#...#.....#............#....#......#.#....
.........#.....#......#.....................................#.............#.......##..#.......#.....
..#..#.#......................#....#..........#...........#............................#....#...#...
#............#..............#...#...##.#.##........#..#....#....#...#...............#.#.............
..#...............#....#...#.........#..#......#..............#........#.........#..................
...........#....................#.....#...#...#..........#..........................................
......................#.............#....#..............#.......#....#.#...........................#
.................#.......#...#.......#...................#.#.......#.....#..........................
#............................#..#..........#..............##..#.............#..........#............
..................#.....................................#..#.#................................#....#
.#......#......#.......#........#.............#..#....................................#.....#..#....
........#..#.......................................#............#.....##.........#...........#..#...
......#..............#..#..#..#.............#..................#...#...........................#....
......................#...#..#.......#.#....................#...........#.......................#...
..#...........#..................#..#....#............................#....#...#.............##..#..
......#....#....#.#.......#...#...............#..........#..........#....#...#....................#.
......................................#................................#...#...........#....#..#....
................#.......................#.......................##.#........#...#............#......
......#..#.................#..........#......................#............#....#..#.......#.........
..##..............................#..#....#.................#........#.#.......#....................
...##.#....#..#..............#......#.......#..#.#.#.......#.#.......##..........#.....#..........##
......#..#....#....#..#.......#............#........###..........#.#..#...........#..........#..#..#
...#........#..........#....................#.........#.....................#...................#...
.#.#........#.................................#..............................#......#...............
.........#...........##........#..............##........#..........#.................#..............
......###....##...#.......................#....................#...................##.#..........#..
..............#....#........................................#.#.............#.....#.#....##.......#.
.........................................................................#.#.##.#..#.....#..........
...#.........#.........#......#..................#....................#..........................#..
..#...#.....#.............#..##..............................#.........##.#..#......................
.....................#...........#.....#......................#..............##..........#.......##.
.............#...............##.................#...#.........##........##.........................#
...#..............................................
.#.#..#...........#.............#.#......##......#
.................................#........##..#.#.
.......................#......#.........#..#......
..#.............#.#................#..............
.........##.##....................................
......#..##.............#....#.........#..........
...............#...........#..............##......
.........#.......#.#..#................#.....#..#.
.#..#.......................#.....#.........#..#..
.................#..#...................##.#..#...
........#...............#...##.............#..##..
...............#..........#.......................
..........#......#..#...................#.......#.
.#.......#..#..........................#..#.......
#.......#............................#............
.....#.#........#.........###............#........
.#.............#............##..#...#...#.#.......
.........#.......#..........#...#........#.#......
...#...........#...............#..................
........#................................#........
............#................................#....
....#........................#..##......#...#.....
.#.##..............#.#..................#.#...#.#.
.............#..........#...................#.....
..##....#....#..#...........#....#.##.............
.#.........#....................#...............#.
........#.#..........#...............#...#...#...#
...............#.#..................#.....##......
.....#....#.##..........#.......#....#...#....#.#.
..#.............#......#............#......#..#...
..#..#............#.#................#..#.........
.........#.......#......#......................#..
..........#.........................#.............
.##.......#...................#.........#..#......
...........#.....##.....#.#...#..........###......
...........#.......#..............................
.........#.......#...#.....#..........#....#..#...
......###.#....#.#..........##....#.......##.#....
.......#.............#..................#.....#...
.............#..........#......#......#...........
...#...........###.....#..#.................#.#...
..................#........##....#......#..#......
........................#.........##.....##.......
........#..#.#....#..........#...#...........#....
.....#........##........#............#..#.........
....................#............#.......#........
.................#.#...#....#.........#..##.......
...........#..........##....#..........#.....#....
..............#..........#.............#..........


    "###.trim_end()[1..]
}
