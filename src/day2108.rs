...
fn part1() {
    let mut readouts = parse_input();
    let mut count = 0;
    for r in readouts.iter_mut() {
        for c in r.output() {
            count += match c {
                '1' => 1,
                '7' => 1,
...
fn part2() {
    ...
    let mut sum = 0;
    for r in readouts.iter_mut() {
        sum += String::from_iter(r.output()).parse::<i32>().unwrap();
    }
...
#[derive(Debug)]
struct Readout {
    signals: [Option<Box<HashSet<char>>>; 10],
    printout: [Option<Box<HashSet<char>>>; 4],
    mapping: Box<HashMap<char, char>>,
}

impl Readout {
    const ZERO: &'static str = "abcefg";
    const ONE: &'static str = "cf";
    const TWO: &'static str = "acdeg";
    const THREE: &'static str = "acdfg";
    const FOUR: &'static str = "bcdf";
    const FIVE: &'static str = "abdfg";
    const SIX: &'static str = "abdefg";
    const SEVEN: &'static str = "acf";
    const EIGHT: &'static str = "abcdefg";
    const NINE: &'static str = "abcdfg";

    fn new() -> Readout {
        const INIT: Option<Box<HashSet<char>>> = None;
        Readout{
            signals: [INIT; 10],
            printout: [INIT; 4],
    ...
    fn output(&mut self) -> Vec<char> {
        if self.mapping.len() == 0 {
            self.calc_mapping();
        }

        let mut v = Vec::new();
        for p in self.printout.iter() {
            if let Some(s) = p {
                let mut chars: Vec<char> = s.iter().map(|c| *self.mapping.get(c).unwrap()).collect();
                chars.sort();
                ...
                match s {
                    ...
                    Self::NINE => v.push('9'),
                    _ => {
                        println!("No match for {}, mapping: {:?}", s, self.mapping);
                        v.push('X')
                    },
                }
            }
    ...
    fn calc_mapping(&mut self) {
        let mut char_counts: HashMap<usize, HashMap<char, i32>> = HashMap::new();
        for sig_hash_opt in self.signals.iter() {
            if let Some(sig_hash) = sig_hash_opt {
                let counts = char_counts.entry(sig_hash.len())
                    .or_insert(HashMap::new());

                for c in sig_hash.iter() {
                    *counts.entry(*c).or_insert(0) += 1;
                }
            }
        }

        let l2 = char_counts.get(&2).unwrap();
        let l3 = char_counts.get(&3).unwrap();
        let l4 = char_counts.get(&4).unwrap();
        let l5 = char_counts.get(&5).unwrap();
        let l6 = char_counts.get(&6).unwrap();

        // a -> appears in l3 but not l2
        let a = Self::key_diff(l3, l2);
        self.mapping.insert(a[0], 'a');

        // c -> appears in 2 l6's, and l2
        for maybe_c in l2.keys() {
            if l6.get(&maybe_c) == Some(&2) {
                self.mapping.insert(*maybe_c, 'c');
                break;
            }
        }

        // b -> appears in l4, not l2, and only once in l5
        // d -> appears in l4, not l2, and all (3) l5's
        let bd = Self::key_diff(l4, l2);
        for maybe_bd in bd {
            match l5.get(&maybe_bd) {
                Some(&1) => self.mapping.insert(maybe_bd, 'b'),
                Some(&3) => self.mapping.insert(maybe_bd, 'd'),
                _ => None,
            };
        }

        // e -> appears in one l5, but not l4, l3, or l2
        for (k, v) in l5 {
            if *v == 1 {
                if l4.get(k) == None && l3.get(k) == None && l2.get(k) == None {
                    self.mapping.insert(*k, 'e');
                    break;
                }
            }
        }

        // f -> appears in 3 l6's, and l2
        for (k, _) in l2 {
            if l6.get(k) == Some(&3) {
                self.mapping.insert(*k, 'f');
                break;
            }
        }

        // g -> appears in all (3) l5's but not l4, l3, or l2
        for (k, v) in l5 {
            if *v == 3 {
                if l4.get(k) == None && l3.get(k) == None && l2.get(k) == None {
                    self.mapping.insert(*k, 'g');
                    break;
                }
            }
        }
    ...
    fn key_diff<K: Copy + Eq + std::hash::Hash, T>(h1: &HashMap<K, T>, h2: &HashMap<K, T>) -> Vec<K> {
        let h1keys: HashSet<&K> = HashSet::from_iter(h1.keys());
        let h2keys: HashSet<&K> = HashSet::from_iter(h2.keys());

        let mut v: Vec<K> = Vec::new();
        for c in h1keys.difference(&h2keys) {
            v.push(**c);
        }
        v
    }
...
fn parse_input() -> Vec<Readout> {
    let mut v: Vec<Readout> = vec![];

    for x in input().lines() {
        let mut readout = Readout::new();
        let parts: Vec<&str> = x.trim().split(" | ").collect();
        for (i, signal) in parts[0].trim().split_whitespace().enumerate() {
            let mut h = Box::new(HashSet::new());
            for c in signal.chars() {
                h.insert(c);
            }
            readout.signals[i] = Some(h);
        }
        for (i, printout) in parts[1].trim().split_whitespace().enumerate() {
            let mut h = Box::new(HashSet::new());
            for c in printout.chars() {
                h.insert(c);
            }
            readout.printout[i] = Some(h);
        }
        v.push(readout);
    }
